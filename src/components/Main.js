            ...
            <div className="SortBarBox">
                ...
                <form>
                    {categories.map((item) => {
                        ...
                        return (
                            <li key={categories.indexOf(item)} style={{listStyleType: 'none'}}>
                                <div className="categories-list-item">
                                    <div className="categories-box">
                                        <input type="checkbox" id={`custom-checkbox-${categories.indexOf(item)}`} name={item.name} value={item.name} checked={checkedState[categories.indexOf(item)]} onChange={() => handleChange(categories.indexOf(item))}/>
                                        <label htmlFor={`custom-checkbox-${categories.indexOf(item)}`}>{item.name}</label>
                                    </div>
                                </div>
                            </li>
                        )
